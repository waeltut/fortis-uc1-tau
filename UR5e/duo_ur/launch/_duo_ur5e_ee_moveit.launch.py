...
def generate_launch_description():
    ...
    joint_controllers_file = os.path.join(
        get_package_share_directory('duo_ur'), 'config', 'ur5e_controllers.yaml'
    )
    ...
    moveit_config = (
        MoveItConfigsBuilder("custom_robot", package_name="duo_ur_ee_moveit_config")
    ...
    rviz_config_path = os.path.join(
        get_package_share_directory("duo_ur_ee_moveit_config"),
        "config",
        "moveit.rviz",
    )
    ...
    rviz_node = Node(
        ...
        # env={
        #     # use host value if present, else fall back
        #     "XDG_RUNTIME_DIR": os.environ.get(
        #         "XDG_RUNTIME_DIR", f"/tmp/xdg-{os.getuid()}"
        #     )
        # },
    )
    ...
    robot_state_publisher = Node(
        ...
        remappings=[('/robot_state_publisher/robot_description','/robot_description')],
        output='screen'
    )
    ...
    # delay_controller_manager_node = RegisterEventHandler(
    #     OnProcessStart(
    #         target_action=robot_state_publisher,
    ...
    # ld.add_action(delay_controller_manager_node)
