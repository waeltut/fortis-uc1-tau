...
def generate_launch_description():
    ...
    moveit_config = (
        MoveItConfigsBuilder("custom_robot", package_name="ur5e_torso_moveit_config")
    ...
    rviz_config_path = os.path.join(
        get_package_share_directory("ur5e_torso_moveit_config"),
        "config",
        "moveit.rviz",
    )
    ...
    cartesian_motion_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["cartesian_motion_controller", "--inactive", "--controller-manager", "/controller_manager"],
        output="screen",
    )
    ...
    delay_cartesian_controller = RegisterEventHandler(
        OnProcessStart(
            target_action=move_group_node,
            on_start=[cartesian_motion_controller_spawner],
        )
    )
