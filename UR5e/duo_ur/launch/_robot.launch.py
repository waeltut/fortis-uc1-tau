...
def generate_launch_description():
    ...
    arg_robot_ip = DeclareLaunchArgument(
        "robot_ip", default_value="192.168.1.102", description="The robot's IP address"
    )
    declared_args = [arg_robot_ip]

    # Robot description
    description_file = PathJoinSubstitution([FindPackageShare("duo_ur"), "urdf", "ur5e_torso.urdf.xacro"])
    ...
    # Robot control
    robot_controllers = PathJoinSubstitution([FindPackageShare("duo_ur"), "config", "controller_manager.yaml"])
    control_node = Node(
    ...
    # Visualization
    rviz_config = PathJoinSubstitution([FindPackageShare("ur5e_torso_moveit_config"),
        "config",
        "moveit.rviz",])
    rviz = Node(
        package="rviz2",
